...
$(function () {
    ...
    var assembliesDataTable, bagsDataTable;
    ...
    var mixderIdent = server.substring(index, server.length);
    index = server.indexOf('/', header + 2);
    ...
    assembliesDataTable = $('#assembliesTable').DataTable();
    bagsDataTable = $('#bagsTable').DataTable();
    ...
    $('#toAssemblyTable tbody').on('click', 'tr', function () {
        if ($(this).hasClass('selected')) {
            ...
            assembliesDataTable.$('tr.selected').removeClass('selected');
    ...
    $('#acceptWeight').click(function () {
        ...
        $.post('/station/dispensary/acceptWeight/:' + selected[0] + '/:' + barcodeText, {itemInfo:itemInfo}, function (data) {
            if (data.info) {
                ...
                _tare();
                assembliesDataTable.row('.selected').remove().draw(false);
                ...
                var rowNode = bagsDataTable.row.add([
                    selectedProductIdent,
                    actualWeight
                ]).draw(false).node();
    ...
    $('#interrupt').click(function () {
        ...
        $.post('/station/dispensary/acceptWeight/:id', itemInfo, function (data) {
            if (data.info) {
                ...
                if (targetWeight > 0) {
                    assembliesDataTable.$('tr.selected').find('input[type="number"]').val(targetWeight);
                } else {
                    assembliesDataTable.row('.selected').remove().draw(false);
                }
            }
    ...
    function checkAssemblyIsFinished() {
        var remainRows = assembliesDataTable.data().count();
    ...
    //     socket = io(server);
    //     socket.on('jobStateChanged', function (jobEventData) {
    //         console.log('jobStateChanged event callbacked');
    //         console.log('options: ');
    //         console.dir(jobEventData);
    //         console.log('jobId: ' + jobId);
    //         if (jobEventData) {
    //             if (jobEventData.mixerIdent === mixderIdent) {
    //                 $.get('/product/getProductByIdent/:' + selectedProductIdent, function (data) {
    //                     if (data.error) {
    //                         $('#errors').append('<li>' + data.error + '</li>');
    //                     }
